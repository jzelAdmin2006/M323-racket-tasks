...
#reader(lib "beginner-reader.rkt" "deinprogramm" "sdp")((modname Tasks1-4) (read-case-sensitive #f) (teachpacks ((lib "image.rkt" "teachpack" "deinprogramm" "sdp"))) (deinprogramm-settings #(#f write repeating-decimal #f #t none explicit #f ((lib "image.rkt" "teachpack" "deinprogramm" "sdp")))))
...
(: list-length ((list-of %element) -> number))
...
(check-expect (list-length animal-list1) 4)
...
; Testfälle
(check-expect (live-dillos dillo-list1) (cons dillo1 empty))
...
(define live-dillos
  (lambda (list)
    (cond
      ...
      (else ; (cons? list)
       (define f (first list))
       (define r (live-dillos (rest list)))
       (if (dillo-alive? f)
           (cons f r)
           r)))))
